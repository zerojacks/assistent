...
def bcd_to_decimal(bcd_array, decimal_places, need_delete,sign):
    ...
    if is_array_all_ffs(bcd_array):
        return "无效数据"

    if need_delete:
        new_array = frame_delete_33H(trans_array)
...
def find_node_by_data_item(data, data_item):
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            result = find_node_by_data_item(value, data_item)
            if result is not None:
                return result
    elif isinstance(data, list):
        for item in data:
            result = find_node_by_data_item(item, data_item)
            if result is not None:
                return result
    return None
...
def get_data_str_delete_33h_reverse(data):
    data_str = "".join([f"{b:02X}" for b in reversed(frame_delete_33H(data))])
    return data_str


def get_data_item_str_delete_33h_order(data):
    data_str = "".join([f"{b:02X}" for b in frame_delete_33H(data)])
    return data_str
...
def hex_array_to_int(hex_array,need_delete):
    if need_delete:
        hex_array = frame_delete_33H(hex_array)
...
def get_subitem_length(data_subitem_elem, splitlength, key):
...
def caculate_item_box_length(item_ele):
    ...
    for item_elem in all_item:
        ...
        item = get_config_xml(item_id, globalprotocol, globregion)
        if item is not None:
...
def parse_time_data(data_array, format_str, need_delete):
    ...
    formatted_data = ""
    if need_delete:
        new_array = frame_delete_33H(data_array)
...
def to_hex_string_reverse(value):
    if isinstance(value, str):
        ...
        return get_data_str_reverser(value)
    else:
...
def to_hex_string_delete_33h_reverse(value):
    if isinstance(value, str):
        ...
        return get_data_str_delete_33h_reverse(value)
    else:
        raise ValueError("Unsupported data type. Expected int or list of ints.")
...
def to_hex_string_with_space(value):
    if isinstance(value, str):
        ...
        return get_data_str_with_space(value)
    else:
        raise ValueError("Unsupported data type. Expected int or list of ints.")
        ...
        if isinstance(item_data, list):
            ...
            if isinstance(sub_data_items, dict):
                ...
                if isinstance(sub_data_items, list):
                    add_data(data_list, f"{item_id}", origial_data, f"{item_id}内容", data_index,sub_data_items)
                else:
                    add_data(data_list, f"{item_id}", origial_data, result_dispriction, data_index)

        elif isinstance(item_data, dict):
            sub_result = []
            prase_data_with_config(item_data, need_delete,sub_result, indent + 1)
