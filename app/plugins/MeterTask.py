...
class MeterTask:
    ...
    def get_sub_oad_info(self, master_oad:int, sub_oad:int):
        ...
        template_element = FrameFun.get_config_xml(item_set, FrameFun.globalprotocol,FrameFun.globregion)
    ...
    def get_ms_len(self, ms_type:int, task_content:bytes, data_result, start_pos):
        pos = 0
        sub_result = []
        if ms_type == MS_TYPE_ALL_USER:
            dis_data_identifier = "全部用户类型"
            FrameFun.add_data(data_result, f"MS",FrameFun.to_hex_string_with_space(ms_type),dis_data_identifier,[start_pos + pos, start_pos + pos + 1])
            return 0
        if ms_type == MS_TYPE_A_SET_OF_USER:
            ...
            for i in range(task_content[0]):
                ...
                FrameFun.add_data(ms_result, f"<第{i+1}组>用户类型",FrameFun.to_hex_string_with_space(task_content[pos]),dis_data_identifier,[start_pos + pos, start_pos + pos + 1])
                pos += 1
            dis_data_identifier = f"用户类型个数:{task_content[0]}"
            FrameFun.add_data(sub_result, f"用户类型个数",FrameFun.to_hex_string_with_space(task_content[0]),dis_data_identifier,[start_pos + pos, start_pos + pos + 1],ms_result)
            dis_data_identifier = "一组用户类型"
            FrameFun.add_data(data_result, f"MS",FrameFun.to_hex_string_with_space(ms_type),dis_data_identifier,[start_pos + pos, start_pos + pos + 1], sub_result)
            return (task_content[0]  + 1)
        ...
        if ms_type == MS_TYPE_A_SET_OF_ADDRESSES:
            ...
            FrameFun.add_data(sub_result, f"用户地址个数",FrameFun.to_hex_string_with_space(task_content[0]),dis_data_identifier,[start_pos + pos, start_pos + pos + 1],ms_result)
            dis_data_identifier = "一组用户地址"
            FrameFun.add_data(data_result, f"MS",FrameFun.to_hex_string_with_space(ms_type),dis_data_identifier,[start_pos + pos, start_pos + pos + 1], sub_result)
            
            return (task_content[0] * 6 + 1)
        ...
        if ms_type == MS_TYPE_A_SET_OF_NUMBERS:
            ...
            for i in range(task_content[0]):
                ...
                dis_data_identifier = f'测量点号:{spot_id}'
                FrameFun.add_data(ms_result, f"<第{i+1}组>测量点号",FrameFun.to_hex_string_with_space(task_content[pos: pos + 2]),dis_data_identifier,[start_pos + pos, start_pos + pos + 1])
                pos += 2
            dis_data_identifier = f"测量点个数:{task_content[0]}"
            FrameFun.add_data(sub_result, f"测量点个数",FrameFun.to_hex_string_with_space(task_content[0]),dis_data_identifier,[start_pos + pos, start_pos + pos + 1],ms_result)
            dis_data_identifier = "一组配置序号"
            FrameFun.add_data(data_result, f"MS",FrameFun.to_hex_string_with_space(ms_type),dis_data_identifier,[start_pos + pos, start_pos + pos + 1], sub_result)
            
            return (task_content[0] * 2 + 1)
        ...
        if ms_type == MS_TYPE_A_RANGE_OF_USER_TYPES:
            ...
            FrameFun.add_data(sub_result, f"区间个数",FrameFun.to_hex_string_with_space(task_content[0]),dis_data_identifier,[start_pos + pos, start_pos + pos + 1],ms_result)
            dis_data_identifier = "一组用户类型区间"
            FrameFun.add_data(data_result, f"MS",FrameFun.to_hex_string_with_space(ms_type),dis_data_identifier,[start_pos + pos, start_pos + pos + 1], sub_result)
            
            return (task_content[0] * 5 + 1)
        ...
        if ms_type == MS_TYPE_A_SET_OF_USER_EVENT_LEVELS:
            ...
            ms_result = []
            for i in range(task_content[0]):
                dis_data_identifier = f'事件等级:{task_content[pos]}'
                ...
                pos += 1
            dis_data_identifier = f"事件等级个数:{task_content[0]}"
            FrameFun.add_data(sub_result, f"事件等级个数",FrameFun.to_hex_string_with_space(task_content[0]),dis_data_identifier,[start_pos + pos, start_pos + pos + 1],ms_result)
        ...
        if ms_type == MS_TYPE_A_SET_OF_USER_PORT_NUMBERS:
            ...
            ms_result = []
            for i in range(task_content[0]):
                dis_data_identifier = f'端口号:{task_content[pos]}'
                ...
                pos += 1
            dis_data_identifier = f"端口号个数:{task_content[0]}"
            FrameFun.add_data(sub_result, f"端口号个数",FrameFun.to_hex_string_with_space(task_content[0]),dis_data_identifier,[start_pos + pos, start_pos + pos + 1],ms_result)
            dis_data_identifier = "一组端口号"
            FrameFun.add_data(data_result, f"MS",FrameFun.to_hex_string_with_space(ms_type),dis_data_identifier,[start_pos + pos, start_pos + pos + 1], sub_result)
            
            return pos

    def analysic_meter_task(self, task_content:bytes, result_list, index, start_pos):
        ...
        for i in range(self.oad_count):
            ...
            pos += 1
            len = self.get_ms_len(ms_type, task_content[pos:], sub_result, pos + index)
            pos += len
            dis_data_identifier = f'<第{i + 1}组>数据采集:{master_oad:08X}'
            FrameFun.add_data(result_list, f"<第{i + 1}组>数据采集","",dis_data_identifier,[index + pos - len, index + pos],sub_result)   
            i += 1
